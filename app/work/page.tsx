...
export default function WorkPage() {
  ...
  return <main><SiteHeader />
    ...
    <section className="work-index">
      {projects.length > 0 ? projects.map((project, index) => <Link className="work-row" href={`/work/${project.slug}`} key={project.slug}><span>0{index + 1}</span><div><p>{project.category} · {project.year}</p><h2>{project.title}</h2><p>{project.summary}</p></div>{project.cover && <Image src={project.cover} alt="" width={420} height={300} />}<span aria-hidden="true">↗</span></Link>) : <div className="work-empty"><p>New case studies are currently being documented.</p><h2>Until then, the earlier portfolio lives on Behance.</h2><a href={site.behance} target="_blank" rel="noreferrer">Open the archive ↗</a></div>}
