...
export type Project = { slug: string; title: string; summary: string; category: string; year: string; role: string; cover?: string; gallery: string[]; tags: string[]; externalUrl?: string; featured: boolean; draft: boolean; order: number; content: string };
...
function parseProject(fileName: string): Project {
  ...
  return { slug, title: data.title ?? slug, summary: data.summary ?? "", category: data.category ?? "Project", year: String(data.year ?? ""), role: data.role ?? "", cover: data.cover || undefined, gallery: Array.isArray(data.gallery) ? data.gallery : [], tags: Array.isArray(data.tags) ? data.tags : [], externalUrl: data.externalUrl || undefined, featured: Boolean(data.featured), draft: Boolean(data.draft), order: Number(data.order ?? 999), content };
}
